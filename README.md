...
旧角色不会因母 Skill 更新立即失效。v1 角色可先运行结构迁移器补到 v2 骨架，再让 Persona.skill 根据证据重蒸馏 `MIND- / EXPR-`、主观背景、`BEHAV-` 行为辨识和 v3 质量记录；迁移器只补显式字段与最新运行脚本，不会替角色编造心理，也不会伪造 v3 测试通过：
...
运行时会分别显示“标签匹配置信度”和“证据完整度”：标签看似匹配但上下文缺失时，综合置信度会自动降低。v3 会在人格文件冻结并计算哈希后，才从母 Skill 中抽取 24 个日常与工作隐藏场景；实际 Runtime 必须逐题生成同一连续对话，并用回答中的逐字片段证明人物判断/情绪/关系与修辞/节奏/背景确实出现。另两个隔离上下文分别生成通用助手和相似人物近失对照，工具把三份回答随机编号为 C1/C2/C3；第四个、未参与生成的隔离上下文只看匿名候选，逐题识别三者并完成六项质量盲评，工具从逐条 0–5 分自行重算总分。任何人格文件变化都会使旧场景和旧分数过期。正式校验会现场重跑仓库内可信检查器，不相信生成目录自报的 `pass`；汇总数字、裸 `pass`、预写答案、同一上下文自评或另换一组对话都不会被验收。
