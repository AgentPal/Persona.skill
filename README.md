...
这一步不会跳过。回复 `1` 使用原名；回复 `2` 后告诉 Agent 自定义名称，也可以直接回复自定义名称，不会重复询问。选好名称后，Agent 会在同一个任务中连续完成调研、生成、校验、修复、测试和启用，不会停在“目录已初始化”“正在整理资料”或“尚未验证”；也不需要你再次回复“继续”。最终回复前还会经过机器完成门禁，只有返回允许交付时才能结束。需要显示的调研进度、问题、校验和完成报告都会以 `角色名：` 开头；内部检索和思考不会单独播报。创建完成后默认在当前会话启用，并在可以安全确定配置位置时持久绑定当前项目；如果只想生成文件，请明确说“只创建不启用”。
