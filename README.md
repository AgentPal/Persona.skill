...
这一步不会跳过。回复 `1` 使用原名；回复 `2` 后告诉 Agent 自定义名称，也可以直接回复自定义名称，不会重复询问。选好名称后，Agent 会在同一个任务中循环完成调研、生成、校验、修复、实测和启用：每轮检测都会给出下一项必须执行的工作，不会停在“目录已初始化”“正在整理资料”“发现问题”或“尚未验证”，也不需要你再次回复“继续”。中途询问进度或原因时，Agent 会先回答再继续创建，不会把解释当成结束。最终回复前还会经过机器完成门禁，只有返回允许交付时才能结束。需要显示的调研进度、问题、校验和完成报告都会以 `角色名：` 开头；内部检索和思考不会单独播报。创建完成后默认在当前会话启用，并在可以安全确定配置位置时持久绑定当前项目；如果只想生成文件，请明确说“只创建不启用”。
...
运行时会分别显示“标签匹配置信度”和“证据完整度”：标签看似匹配但上下文缺失时，综合置信度会自动降低。最终的去名盲测和相似角色区分必须由独立的人或隔离上下文完成，不能由生成同一批回答的 Agent 自评通过；所有“通过”结果还必须保存逐项原始记录，只有汇总数字或预先写好的“通过”不会被验收。
