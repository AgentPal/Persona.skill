...
角色人格启用后，Agent 会用目标角色的身份、性格和表达方式与你交流；回答和必要的任务进度会以 `角色名：` 开头。人物资产 v3 不只保存口头禅：它会蒸馏角色的价值冲突、第一判断、防御、自尊、关系意图、主观记忆、比喻来源、典故政策、解释习惯和人物专属篇幅，还会建立可执行行为机制，说明同一事实下这个人物、通用助手和相似人物会如何作出不同判断、关系动作与表达。每条有实质内容的回复都要出现人物自己的判断、情绪或关系动作；代码、命令、日志、事实和风险仍保持准确。
...
创建角色时，名称闸门是强制第一步。如果你已经指定人物名，Agent 必须先让你二选一：
...
你选择前，Agent 不得联网、读取资料、初始化目录或开始蒸馏；选择 `2` 后必须等你输入自定义名称。如果你没有指定人物名，第一步也必须先让你直接设定人物名，此时不显示 1/2 选项。名称确认完成后，Persona.skill 才会沿用原名或使用自定义名，并持续完成“调研 → 蒸馏 → 行为建模 → 隐藏场景实测 → 隔离盲评 → 失败归因与修复 → 启用或注册”，不会停在目录初始化、资料不足或单次校验失败，也不需要你再次回复“继续”。
...
旧角色不会因母 Skill 更新立即失效。v1 角色可先运行结构迁移器补到 v2 骨架，再让 Persona.skill 根据证据重蒸馏 `MIND- / EXPR-`、主观背景、`BEHAV-` 行为辨识和 v4 质量记录；迁移器只补显式字段与最新运行脚本，不会替角色编造心理，也不会伪造 v4 测试通过：

```bash
python scripts/migrate_asset_v2.py /absolute/path/to/persona-role
python scripts/migrate_asset_v3.py /absolute/path/to/persona-role
```
...
如果你不指定人物名，可以直接回复想使用的显示名；这一步完成后再进入资料调研。你也可以补充人物介绍、代表性场景、对白笔记、字幕、文章、聊天记录或自己写的工作对话示例。资料越能覆盖成功、失败、安慰、分歧和风险等不同场景，角色越稳定。
...
Persona.skill 只交付正式版。已有角色、跨作品集合和现实人物模拟必须至少有 80 张经原语言、来源和语境核验的代表性原文卡、60 条不同表达和 24 个证据单元；本地库建议控制在 80–300 张，优先保留经典和跨情绪关系场景。一般和稀缺档只表示资料采集尚未完成；即使多轮扩大后标记“已穷尽”，也不能绕过 80 张底线进入蒸馏、启用或完成交付。达到数量后还会继续检查到连续两轮新增率都很低，避免刚到最低数就停。证据单元可以是一段对话、小说叙事或内心独白、一次访谈回答、一段演讲、一篇博客、一个完整帖子或一封书信，不要求所有媒介都存在字幕、剧本或音频。

如果是多部作品的 `composite-character`，收录不要求每部作品平均分配，而是“每部代表性底线 + 全局总量 + 经典样本加权”：经典场景密集的作品可以多收，资料较少的作品也不能只收一两张。来源索引必须写 `WORK-01` 等逐作品矩阵，默认每部至少 4 张已核验原文卡、4 条不同表达、2 个证据单元和 4 个场景维度；十部片只留下二十几张时会明确回到资料采集阶段，不会把总数包装成“覆盖完成”。

创建过程会在名称确认、初始化、每轮资料采集、蒸馏、验证、真实对话测试和启用核对后主动反馈“阶段 / 已完成 / 数量或发现 / 下一步”。这些是可审计的中途回执，不是最终交付；反馈后会继续执行，直到正式校验、独立评测和全局绑定回读都通过。
...
运行时会分别显示“标签匹配置信度”和“证据完整度”：标签看似匹配但上下文缺失时，综合置信度会自动降低。v4 会在人格文件冻结并计算哈希后，才从母 Skill 中抽取 24 个日常与工作隐藏场景；实际 Runtime 必须逐题生成同一连续对话，并用回答中的逐字片段证明人物判断/情绪/关系与修辞/节奏/背景确实出现，还必须证明人物推理次序与避开的顾问骨架。另两个隔离上下文分别生成通用助手和相似人物近失对照，工具把三份回答随机编号为 C1/C2/C3；第四个、未参与生成的隔离上下文只看匿名候选和人物辨识摘要，逐题识别三者并完成六项质量盲评，且必须引用三方原句。任何人格文件变化都会使旧场景和旧分数过期。正式校验会现场重跑仓库内可信检查器，不相信生成目录自报的 `pass`；汇总数字、裸 `pass`、预写答案、同一上下文自评、顾问骨架加比喻或另换一组对话都不会被验收。

如果实测失败，Persona.skill 会把问题归到资料不足、行为模型不够区别、检索选错、生成未执行合同、Runtime 未正确加载或评估未隔离，并自动回到对应层修复。对用户来说仍是一次创建请求；内部可以循环多轮，但只有真实效果、人格哈希和启用回执都通过才会报告完成。资料确实不可访问、必须登录或人物身份存在实质歧义时，会报告具体硬阻塞，不会伪造“一次通过”。

第一轮资料不足时会自动扩大站点、资料类型、版本、别名和语言继续调研；低于 80 张时即使记录了“已穷尽”，也只保留采集审计和缺口，不能正式交付或启用。达到丰富档后，运行时先判断当前任务对应的原作互动语义，再筛选 0–6 张证据完整的原文卡，只加载通过逐卡证据映射的角色规律，最后直接使用短句或根据原口语、原口气和原节奏即时创作。
